...
public class RedMPanning extends JFrame {
    private static final String VERSION = "1.104";
    ...
    private static final String REDM_WINDOW_TITLE = "RedM™";
    private static final String REDM_PROCESS_NAME = "RedM.exe";
    private JLabel gameStatusLabel;
    private static final String REDM_IDENTIFIER = "RedM";
    private JLabel overlayStatusLabel;
    ...
    private boolean isRedMRunning() {
        try {
            ProcessBuilder pb = new ProcessBuilder(
                "cmd.exe", "/c", 
                "wmic process where \"commandline like '%RedM%'\" get processid /format:list"
            );
    ...
    private void updateOverlayStatus() {
        if (overlayStatusLabel != null) {
            String status = running ? "Auto-Panning: Active" : "Auto-Panning: Stopped";
            if (running && scriptThread != null && scriptThread.isAlive()) {
                status += " (Panning)";
            }
            overlayStatusLabel.setText(status);
            overlayStatusLabel.setForeground(running ? new Color(141, 241, 138) : new Color(200, 200, 200));
        }
    }
    ...
    private void setupOverlay() {
        ...
        JPanel overlayPanel = new JPanel() {
            ...
            protected void paintComponent(Graphics g) {
                ...
                GradientPaint gradient = new GradientPaint(
                    0, 0, new Color(28, 32, 38, 220),
                    0, getHeight(), new Color(41, 46, 56, 220)
                );
        ...
        overlayPanel.setLayout(new BorderLayout(10, 10));
        overlayPanel.setOpaque(false);
        overlayPanel.setBorder(BorderFactory.createEmptyBorder(12, 15, 12, 15));
        
        JPanel textPanel = new JPanel();
        textPanel.setLayout(new BoxLayout(textPanel, BoxLayout.Y_AXIS));
        textPanel.setOpaque(false);
        
        JLabel titleLabel = new JLabel("RedM Auto Panner");
        titleLabel.setFont(new Font("Segoe UI", Font.BOLD, 14));
        titleLabel.setForeground(new Color(200, 200, 200));
        titleLabel.setAlignmentX(Component.LEFT_ALIGNMENT);
        
        overlayStatusLabel = new JLabel("Auto-Panning: Waiting");
        overlayStatusLabel.setFont(new Font("Segoe UI", Font.PLAIN, 13));
        overlayStatusLabel.setForeground(Color.WHITE);
        overlayStatusLabel.setAlignmentX(Component.LEFT_ALIGNMENT);
        
        textPanel.add(titleLabel);
        textPanel.add(Box.createVerticalStrut(4));
        textPanel.add(overlayStatusLabel);
        
        overlayPanel.add(textPanel, BorderLayout.CENTER);
        
        overlay.add(overlayPanel);
        overlay.setSize(220, 70);
